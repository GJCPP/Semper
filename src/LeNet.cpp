...
LeNet::LeNet(std::string _data_dir, int epoch, int64_t scale, int64_t max_value, uint64_t rho_inv, uint64_t sec_param)
    : CNN("LeNet", _data_dir + "/LeNet", epoch, scale, max_value, rho_inv, sec_param) {

    set_timer("LeNet load & commit");
    ...
    std::vector<std::vector<int>> conv_layers = {{1}, {2}};
    int ind_pool = 1;
